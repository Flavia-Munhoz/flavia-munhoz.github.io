...
public class SugestaoDAO {
	...
	public List<Sugestao> getSugestao() throws SQLException {
		...
		try {
			...
			while (rset.next()) {
				...
				string = string + " - Nome: " + rset.getString("nome") + " - Email: " + rset.getString("email")
						+ "Sugestão:   " + rset.getString("sugestao") + "\n";
	...
	public void updateS(int id, Sugestao sugestao) throws SQLException {
		String sql = "UPDATE sugestao SET  nome= ?,email= ?,sugestao= ?" + "WHERE id_sugestao= ?";
